...
def update_map(map_data, per_capita, sel_day, small_pop):
    ...
    cases_label = ["Cases : " + human_format(total_cases_sel_date),
                   dbc.Badge(str('{:+.0%}'.format(cases_variance)), className="mr-3 position-relative float-right",
                             id='badge_case',
    ...
    lbl_deaths = ["Deaths : " + human_format(total_deaths_sel_date),
                  dbc.Badge(str('{:+.0%}'.format(death_variance)), className="mr-3 position-relative float-right",
                            id='badge_death',
                            style={'backgroundColor': '#508caf', 'width': '50px'}),
        ...
        dbc.CardHeader([
            ...
            dbc.Modal([
                ...
                dbc.ModalBody([
                    ...
                    html.Br(),
                    dbc.Row([
                        dbc.Col(html.H5('Fitted Development Factors', className='font-weight-bold'),
                                className='col-12 col-md-6'),
                        dbc.Col(
                    ...
                    dbc.Row([
                        ...
                        dbc.Col(
                            dcc.Dropdown(options=model_factors,
                                         id='factors_dd', value=model_factors[0]['value'], multi=False,
                                         style={'fontSize': '100%'}, clearable=False),
                            className='col-12 col-md-6')
                    ], align='center'),
                    ...
                    dbc.Row([
                        ...
                        dbc.Col(
                            dcc.Dropdown(options=[{'label': 'Devt Time', 'value': 'devt_time'},
                                                  {'label': 'Country Groups', 'value': 'country group'}],
                                         id='residuals_dd', value='devt_time', multi=False, style={'fontSize': '100%'},
                                         clearable=False),
                            className='col-12 col-md-6')
                    ], align='center'),
        ...
        dbc.CardBody([
            ...
            dbc.Row([
                dbc.Col([
                    ...
                    html.A('COVID-19 Projection Model',
                           href='https://medium.com/@alban.tranchard/covid-19-cases-projections-4d09fe655303?source=friends_link&sk=a9506c925ca80fbd51a95727c8b7a741',
                           target='_blank'),
                    html.P(' '),
...
def update_fit_curve(country):
    ...
    fig = px.line(plot_data, x='devt_time', y='link_ratio', template='plotly_white',
...
def update_timeline_plots(n, region, subregion, country, area, split, top_limit, first_time):
    ...
    if split:
        dff_agg = dff.loc[:, [split, 'date', 'devt_time', 'data', 'population', 'cases', 'cases_inc']].groupby(
            [split, 'date']).sum() \
        ...
        df_combined = dff.loc[:, [split, 'data', 'cases', 'date', 'population']].groupby([split, 'data', 'date']).sum() \
            .reset_index().sort_values(by=[split, 'date'], ascending=True)
        df_combined = df_combined.groupby([split, 'data']).max().reset_index().sort_values([split, 'cases'],
                                                                                           ascending=True)
